...
from graphs.state import InterviewPrepInput, InterviewPrepOutput
...
def interview_prep_node(
    state: InterviewPrepInput,
    config: RunnableConfig,
    runtime: Runtime[Context]
) -> InterviewPrepOutput:
    """
    title: 面试准备
    desc: 为通过筛选的候选人生成针对性面试题目和邀约邮件，并发送邮件通知
    ...
    interview_questions: List[str] = result.get("interview_questions", [])
    invitation_content: str = result.get("invitation_content", "")
    ...
    if candidate_email and invitation_content:
        candidate_name = parsed_info.get("name", "候选人") if isinstance(parsed_info, dict) else "候选人"
        
        # 替换邮件中的占位符
        email_content = invitation_content.replace("[候选人姓名]", candidate_name)
